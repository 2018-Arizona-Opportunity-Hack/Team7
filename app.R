...
formatCSV <- function(nops_csv, qa_csv){
  ...
  #nops_data <- read.csv("/Users/Tommy/Documents/Git/Hackathon/unzippedPDFS1548789820.00079/results.csv",stringsAsFactors=F,header=T,sep=";")
  ...
  for(i in 1:nrow(data)){
    for(j in 1:length(answers)){
      #print(formatC(as.character(answers[i,j]), width = 5, format = "d", flag = "0"))
      answerVal <- formatC(as.character(answers[i,j]), width = 5, format = "d", flag = "0");
      for(k in 1:nchar(answerVal)){
        if(substr(answerVal,start=k,stop=k) == 1){
          user[index] <- i
          question[index] <- j
          answer[index] <- k
          index <- index + 1
  ...
  #qa_data <- read.csv("/Users/Tommy/Documents/Git/Hackathon/unzippedPDFS1548789820.00079/QuestionData.csv")
  qIndex <- 1
  comp <- qa_data[1,2]
  
  for (row in 1:(nrow(qa_data))) {
    if (qa_data[row,2] == as.character(comp)) {
      qa_data[row,1] <- qIndex
    } else {
      comp <- qa_data[row,2]
  ...
  #qa_data[,1] <- as.numeric(as.factor(qa_data[,2]))
  
  questions_list <- unique(as.character(qa_data[,2]))
  answers_list <- split(qa_data,qa_data[,1])
