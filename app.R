...
formatCSV <- function(nops_csv, qa_csv){
  ...
  #nops_data <- read.csv("/Users/Tommy/Desktop/nops_eval_sc.csv",stringsAsFactors=F,header=T,sep=";")
  
  # Set up the data
  info <- nops_data[,c("registration","name","id","scan")]
  answers <- nops_data[,grep("answer",colnames(nops_data))]
  ...
  #qa_data <- read.csv("/Users/Tommy/Desktop/QuestionData.csv")
...
server <- function(input, output) {
  ...
  observeEvent(input$pdfInput, {
    ...
    if (is.null(infile)){
      ...
      clean_data <- formatCSV(nops_csv = paste0("../",fp,"/results.csv"), qa_csv = paste0("../",fp,"/QuestionData.csv"))
